"""Write-through cache helpers for course landing pages.

Pure infrastructure layer over Redis: serialize/store/evict already-built
payload dicts keyed by landing slug or the homepage. Intentionally imports NO
service module so higher layers can depend on it without circular imports.

All operations degrade gracefully: if the cache is disabled or Redis is
unavailable, reads return None and writes/evicts become no-ops (callers fall
...
def cache_set(key: str, payload: dict, ttl: Optional[int] = None) -> None:
    ...
    if client is None:
        return
    try:
        client.setex(key, ttl or settings.LANDING_CACHE_TTL, json.dumps(payload))
