...
- **Background jobs**: asyncio tasks (expire orders, broadcast dispatch)
...
│   │   │   ├── jobs/                    # Background jobs (expiry, broadcast)
...
| Method | Path | Mô tả |
|--------|------|-------|
| `GET` | `/public/funnels/{slug}` | Lấy dữ liệu landing page (có cache) |
...
- Bao gồm: checkout flow, discount engine, broadcast, lead capture, in-process landing cache, revenue analytics, lucky draw, media library, URL redirects,...
...
Runbook E2E kiểm tra luồng chính, 2 cách chạy:
- **Docker Compose** — dùng cho CI/release verification (recommend)
- **Host/dev** — chạy với Postgres local, không cần Docker
